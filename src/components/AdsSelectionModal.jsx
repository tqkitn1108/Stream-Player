...
function AdsSelectionModal({
  ...
  }, [isOpen, programStartTime, existingAds]); // Find next available time slot for ads
  ...
  const filteredAds = adsList.filter((ad) => {
    return (
      filterCategory === "all" || ad.categoryId === parseInt(filterCategory)
    );
  });
  const confirmAddAds = () => {
    ...
    const totalAdsDuration = adsToAdd.reduce((total, adData) => {
      const duration = dayjs(adData.endTime).diff(
        dayjs(adData.startTime),
        "second"
      );
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div
        ...
        onClick={closeModal}
      ></div>
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-4xl z-10 max-h-[90vh] overflow-y-auto">
        {" "}
            ...
            <div className="max-h-80 overflow-y-auto border border-gray-700 rounded p-3">
              {loadingAds ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {filteredAds.map((ad) => {
                    const isSelected = selectedAds.find(
          ...
          <div>
            ...
            <div className="mb-4">
              <label className="block text-gray-200 mb-2">
                Thời gian bắt đầu quảng cáo đầu tiên
              </label>{" "}
              <input
                type="datetime-local"
                value={adStartTime}
                ...
                min={
                  programStartTime
                    ? dayjs(programStartTime).format("YYYY-MM-DDTHH:mm:ss")
                    : ""
                }
              />
              <p className="text-xs text-gray-400 mt-1">
            ...
            {selectedAds.length > 0 && (
              <div>
                <label className="block text-gray-200 mb-2">
                  Quảng cáo đã chọn ({selectedAds.length})
                </label>
